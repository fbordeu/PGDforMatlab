function [AII,AIV,AJJ,AJV,AW,Aoffsets,BII,BIV,BW,Boffsets,Nrows,Ncols] = PGD_DETAILLED_ASSEMBLE_2(DOM_NAME,COORDS,U_FIELDS,T_FIELDS,K_FIELDS,INTEG_PARAM,varargin)
LHS = [];
RHS = [];
if ~isempty(K_FIELDS)
...
if has_lhs
    ...
    for t = 1:numel(LHS)
        ...
        for i=1:gen_terms(t)
            AII{idx_AA} =  T_INTERP.PCTableIndex{find(INTEG_PARAM==T_INTERP.PCIntegrationParam),term.test_diff_symbol};
            AIV{idx_AA} = T_INTERP.PCTableValues{find(INTEG_PARAM==T_INTERP.PCIntegrationParam),term.test_diff_symbol};
            AJJ{idx_AA} = U_INTERP.PCTableIndex{find(INTEG_PARAM==U_INTERP.PCIntegrationParam),term.unknown_diff_symbol};
            AJV{idx_AA} = U_INTERP.PCTableValues{find(INTEG_PARAM==U_INTERP.PCIntegrationParam),term.unknown_diff_symbol};
            AW{idx_AA}  = tmp(:,i);
...
if has_rhs
    ...
    for t = 1:numel(RHS)
        ...
        BII{t} =T_INTERP.PCTableIndex{find(INTEG_PARAM==T_INTERP.PCIntegrationParam),term.test_diff_symbol};
        BIV{t} = T_INTERP.PCTableValues{find(INTEG_PARAM==T_INTERP.PCIntegrationParam),term.test_diff_symbol};
        BW{t}  = tmp;
